...
////////////////	mhwon 추가 start
	...
	$('#ring_img').click(function(){
		...
		if($('#div_ring').is(":visible"))
			...
			//location.href = "car_pickup_list.myt?menu=car&gbn=3&id=" + login_id;
	...
	$(document).on('click', '#close_ring', function(){ 
	...
	$(document).on('click', '#btn_ring_del', function(){ 
		...
		//var sel = document.querySelector('input[name="rad_del"]:checked');
		//console.log(sel);
		
		var resv_no = $('input[name=rad_del]:checked').val();
		//var resv_no = document.querySelector('input[name="rad_del"]:checked').value;
		//resv_no = document.querySelector('input[name="rad_del"]:checked').value;
		...
		if(resv_no == null){
			...
			if(confirm("해당 내역을 삭제하시겠습니까?")){
				//	삭제로직
				$.ajax({ // key value 형식
					url : "car_pickup_del.myt?menu=car&gbn=4&id=" + login_id + "&resv_no=" + resv_no ,
					success : function(re){
						div_ring.innerHTML = re;
